...
\newcommand{\hmwkTitle}{Cryptanalysis of the Hill Cipher} % Assignment title
...
\\
\begin{algorithmic}[1]
\REQUIRE{Ciphertext $Y_1,Y_2,...,Y_n$ and classification of ciphers and plaintext in classes $C_{i}$ of reduction modulo 2}\\
\STATE Find the $d+1$ classes with the highest probability that a block appears for the plaintext.
\FORALL{$d+1$ classes $C_{i}$ take the $block_{i}$ associated modulo 26, $i \in \{0,1,...,d+1\}$}
	\STATE find the cipher modulo 2 called $Y_{i}^2$ associated to the $block_{i}$
\ENDFOR
	\REPEAT
	\STATE{ take one of the string $k \in C_{i}$, called $X_{i_{k}}{26}$ that match with $Y_{i}^2$,$i \in \{0,1,...,d+1\}$}
