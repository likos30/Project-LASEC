...
From diverse calculation we always get $0.5^n$ so E(\# all matching) is always equals to $(\# segments in reference) \times (\#segment in plaintext) \times 0.5^n$.\\
...
what we want is to enhance the possible FFT on a table called $n_y$ which contains the number of times $k$ where each cipher $y$ appears. So it is a table containing numbers $\in \mathbb{R} $.
...
If you suppose all our entries in $n_y$ are non null and the , 
